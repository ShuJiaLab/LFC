function [PSFpath,FLFMpath,varargout] = Decon3D(PSFpath,FLFMpath,Reconpath,Iter,Centers,...
...
for ii = 1:1:length(FLFMfiles)
    ...
    if ~exist(reconfilename,"file") || isreplace
        ...
        % This line adjust the intensity along the axial direction
        % Xguess_norm_re = Xguess_norm_re./max(Xguess_norm_re,[],[1,2]).*...
        %                  reshape(smooth(squeeze(max(Xguess_norm_re,[],[1,2])),10,"loess"),1,1,[]);
        Xguess_norm_re = Xguess_norm_re./max(Xguess_norm_re,[],[1,2]).*...
                         reshape(smooth(squeeze(max(Xguess_norm_re,[],[1,2])),16,"loess"),1,1,[]);
